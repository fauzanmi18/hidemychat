    ...
    <div class="flex flex-col items-center mx-4">
        <div class="mx-4 mb-4 flex lg:flex-row flex-col gap-2 w-full">
            ...
            <div class="p-4 bg-white flex flex-col rounded-lg w-full lg:w-1/2">
                ...
                <textarea v-model="showChat" ref="showChatTextArea" class="bg-white border rounded-lg p-4" rows="7" readonly></textarea>
            </div>
        </div>

        <button @click="showChatDecode" id="showBtn">
            Show Chat
        </button>
    </div>
</template>
...
const showChatTextArea = ref(null)

const showChatDecode = () => {
    if(inputChat.value?.length > 0 || inputChat.value.trim() !== ''){
        console.log(atob(inputChat.value))
        showChat.value = atob(inputChat.value)

        const textarea = showChatTextArea.value;
        textarea.focus();
    }
}

const copyToClipboard = () => {
    if(showChat.value?.length > 0 || showChat.value.trim() !== ''){
        isCopied.value = true

        const textarea = document.createElement('textarea');
        textarea.value = showChat.value;
